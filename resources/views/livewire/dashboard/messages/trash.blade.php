<div>
    <div class="card">
        <div class="card-header">
            <h4 class="card-title"><i class="fas fa-trash-alt mr-2 text-danger"></i>{!! __('messages.trash') !!}</h4>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                <th style="width: 100px;">{!! __('messages.user') !!}</th>
                                <th>{!! __('messages.subject') !!}</th>
                                <th style="width: 150px;">{!! __('messages.date') !!}</th>
                                <th style="width: 180px;" class="text-right">{!! __('messages.actions') !!}</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr wire:key="msg-{{ $message->id }}">
                                    ...
                                    <td class="text-right">
                                        <button wire:click="restore({{ $message->id }})"
                                            class="btn btn-outline-success btn-sm " title="{!! __('messages.restore') !!}">
                                            <i class="fas fa-undo"></i>
                                        </button>
                                        <button wire:click="confirmPermanentDelete({{ $message->id }})"
                                            class="btn btn-outline-danger btn-sm" title="{!! __('messages.delete_forever') !!}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-5 text-muted">
                                        <i class="fas fa-trash fa-3x mb-3 opacity-25"></i>
                                        <p class="mb-0">{!! __('messages.no_trash_messages') !!}</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="modal-content border-0 shadow-lg" style="border-radius: 15px;">
                <div class="modal-header bg-primary text-white"
                    ...
                    <h5 class="modal-title font-weight-bold" id="messageDetailsModalLabel">
                        <i class="fas fa-envelope-open-text mr-2"></i> {!! __('messages.messages_details') !!}
                    </h5>
                ...
                <div class="modal-footer border-0">
                    <button type="button" class="btn btn-outline-secondary px-4" data-dismiss="modal">{!! __('general.close') !!}</button>
                </div>
            </div>
        </div>
    </div>
