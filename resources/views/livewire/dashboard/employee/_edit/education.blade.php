            ...
            @foreach ($educationItems as $index => $row)
                <tr wire:key="row-{{ $index }}" class="border-bottom">
                        ...
                        <div class="mt-2 d-flex justify-content-center gap-2">
                            @if ($row['certification'] && !$row['new_certification'])
                                <div class="text-center">
                                    <small class="d-block text-muted" style="font-size: 0.7rem">{!! __('employees.current') !!}</small>
                                    <img src="{!! asset('uploads/employeesCertifications/' . $row['certification']) !!}" 
                                        class="premium-photo-preview shadow-sm" style="width: 50px; height: 50px; border-radius: 6px" />
                                </div>
                            @endif

                            @if ($row['new_certification'])
                                <div class="text-center">
                                    <small class="d-block text-primary" style="font-size: 0.7rem">{!! __('employees.new') !!}</small>
                                    <img src="{!! $row['new_certification']->temporaryUrl() !!}" 
                                        class="premium-photo-preview shadow border-primary" style="width: 50px; height: 50px; border-radius: 6px" />
                                </div>
                            @endif
                        </div>
                    </td>
                    ...
                    <td class="p-2 text-center align-middle">
                        <button type="button" wire:click.prevent="removeEducation({{ $index }} , {{ $row['id'] }})"
